...
:EXPORT_HUGO_IMAGES: /posts/20240818_hosting-a-valheim-plus-server-on-a-vps-with-podman-and-docker-compose/valheimreadme.png
:EXPORT_HUGO_MENU:
:END:
...
#+end_quote
Valheim is stunning game that really captured my attention and was an excellent escape through a long winter few years back, I describe it as Viking Minecraft. The game supports multiplayer and people have figured out how to run it headless so there's a persistent world on a server so people can play and build asynchronously. It's quite fun to play with some friends when a simple resource gathering trip turns into a harrowing boondoggle.
However, there are a lot of small quality of life (I'm not a 'real gamer' just cos-playing) improvements that could make the game more enjoyable. These (and many more) things have coalesced into a mod-pack called Valheim Plus.
**** Valheim Plus
...
*** The Setup
I went through a couple different approached but I'm happy with a =docker-compose.yaml=, a =valheim.env= file, and the =valheim_plus.cfg= file. It is a bit annoying that all three files in are their separate file types but that's just modern development.
**** Documentation and Version Control
I have a private git server for my self-hosted infrastructure so I can commit secrets in .env files and not worry about leaking them accidentally. I wrote up a readme file, screenshot included to show how nice self-hosted Forgejo is and how it pays to do documentation for your own sake.
#+begin_export hugo
...
- The Valheim Plus documentation will tell you to place the =valheim_plus.cfg= file in the =BepInEx= folder but that made version controlling it a bit janky so I map it directly into the location inside the container where it's expected.
...
- ~networks: reverse-proxy~ refers to Caddy that is proxying the requests coming to the Supervisor, which is a small web-app that allows control of the pieces of the tech stack.
#+begin_export hugo
...
#+end_export
**** Version Control
With this setup you only have to version control three files: =docker-compose.yaml=, =valheim.env= (don't share publicly), and =valheim_plus.cfg= (I suggest creating a readme and version controlling that too).
**** Syncthing
This is probably the odd-ball part here, it's basically the easiest way for me to create an "on-site backup" for my "off-site server" (VPS). This really isn't in scope but wanted to throw it out there as a cheaper place to backup those frequent and bulky (for cloud storage but light for home) backups that the container stack creates.
*** Takeaway
There are great tools out there to self-host things for fun and to be in control of your data (more on that in the future too). But being able to recreate, reproduce, with reliability requires a little planning and some documenting. This blog doesn't have comments but happy to discuss further on [[https://phanpy.social/#/fosstodon.org/a/243697][Mastodon]].
